...
BEGIN
  -- Posts RLS
  BEGIN
    CREATE POLICY "Public can view all posts" ON public.posts FOR SELECT USING (auth.role() = 'authenticated');
  ...
  -- Likes RLS
  BEGIN
    CREATE POLICY "Public can view all likes" ON public.post_likes FOR SELECT USING (auth.role() = 'authenticated');
  EXCEPTION WHEN duplicate_object THEN NULL; END;
  ...
  -- Comments RLS
  BEGIN
    CREATE POLICY "Public can view all comments" ON public.post_comments FOR SELECT USING (auth.role() = 'authenticated');
  EXCEPTION WHEN duplicate_object THEN NULL; END;
