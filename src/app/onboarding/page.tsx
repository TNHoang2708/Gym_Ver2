...
export default function OnboardingPage() {
  ...
  // Global Enter key listener to continue
  useEffect(() => {
    function handleKeyDown(e: KeyboardEvent) {
      // Ignore if user is typing in an input (the input has its own Enter handler)
  ...
  }, [currentStep, answers, multiSelected, numInput, saving])

  return (
    <div className="relative min-h-[100dvh] bg-background flex items-center justify-center py-12 md:py-16">
      {/* Ambient Background */}
      ...
      <div className="relative z-10 w-full max-w-xl mx-auto flex flex-col bg-[#050505]/80 backdrop-blur-2xl border border-white/5 rounded-[32px] p-6 md:p-10 shadow-2xl">
        {/* Header / progress */}
        <div className="w-full pb-8 border-b border-white/5 mb-8">
      ...
      <div className="w-full overflow-visible">
        <AnimatePresence mode="wait" custom={direction}>
          <motion.div 
            ...
            className="pt-4 pb-6"
          >
            <h1 className="text-3xl md:text-4xl font-bold tracking-tight mb-4 text-foreground" style={{ fontFamily: 'var(--font-space-grotesk)' }}>
              {step.question}
            </h1>
          {step.subtitle && (
            <p className="text-base text-muted-foreground mb-8">{step.subtitle}</p>
          )}
              ...
              {step.options.map((opt) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      'w-full flex items-center gap-5 px-6 py-5 rounded-2xl border text-left transition-all duration-300',
                      isSelected
                        ? 'bg-gold/10 border-gold/40 text-foreground scale-[1.02] shadow-[0_0_20px_rgba(212,175,106,0.15)]'
                        : 'bg-secondary/50 border-white/5 text-muted-foreground hover:border-white/20 hover:text-foreground hover:bg-secondary'
                    )}
                  >
          ...
          {step.type === 'multi' && step.options && (
            <div className="space-y-3 mt-8">
              <p className="text-sm text-muted-foreground mb-4 uppercase tracking-wider font-semibold">Select all that apply</p>
              {step.options.map((opt) => {
            ...
            <div className="mt-6">
              <div className="relative">
                <input
                  ...
                  className="w-full bg-secondary border border-border rounded-2xl px-6 py-5 text-3xl font-bold text-foreground placeholder:text-muted-foreground/30 focus:outline-none focus:ring-2 focus:ring-gold/50 focus:border-gold/50 pr-24 transition-all shadow-inner"
                  autoFocus
                />
                {step.unit && (
      ...
      {/* Navigation buttons */}
      <div className="pt-8 mt-4 w-full">
        <div className="flex gap-4">
          {currentStep > 0 && (
            <button
              onClick={handleBack}
              className="flex items-center gap-2 px-6 py-4 rounded-full border border-white/10 text-muted-foreground hover:text-foreground hover:border-white/20 hover:bg-secondary/50 transition-all font-medium"
            >
          ...
          <button
            ...
            className={cn(
              'flex-1 flex items-center justify-center gap-2 py-4 rounded-full font-bold text-lg transition-all duration-300',
              canProceed() && !saving
                ? 'bg-gradient-to-r from-red-600 via-orange-500 to-gold hover:brightness-110 text-white shadow-[0_0_20px_rgba(212,175,106,0.2)] scale-[1.01]'
